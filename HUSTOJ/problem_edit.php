  ...
  <div class="container">
    ...
    <form method=POST action=problem_edit.php>
      ...
      <p align=left>
        ...
        <textarea name=front style="width:100%;" rows=1><?php echo htmlentities($row['front'],ENT_QUOTES,"UTF-8")?></textarea><br>
      </p>

      <p align=left>
        <?php echo "<h5>"."뒤에 붙일 코드"."</h5>"?>
        <textarea name=rear style="width:100%;" rows=1><?php echo htmlentities($row['rear'],ENT_QUOTES,"UTF-8")?></textarea><br>
      </p>
    ...
    <?php
      ...
      //CSL
      $sql = "UPDATE `problem` SET `title`=?,`time_limit`=?,`memory_limit`=?,`description`=?,`input`=?,`output`=?,`sample_input`=?,`sample_output`=?,`hint`=?,`source`=?,`spj`=?,`front`=?,`rear`=?,`bann`=?,`in_date`=NOW() WHERE `problem_id`=?";

      @pdo_query($sql,$title,$time_limit,$memory_limit,$description,$input,$output,$sample_input,$sample_output,$hint,$source,$spj,$front,$rear,$bann,$id);
      //CSL
