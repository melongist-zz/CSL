...
sed -i "s#feed_server_path = './sample/'#feed_server_path = './src/'#" /var/www/html/spotboard/src/config.js
sed -i "s#'award_slide.json' :'./sample/award_slide.json'#'award_slide.json' :'./src/award_slide.json'#" /var/www/html/spotboard/src/config.js
sed -i "s#animation          : false#animation          : true#" /var/www/html/spotboard/src/config.js
...
echo "------" | tee -a domjudge.txt
echo "http:/localhost/spotboard/src/" | tee -a domjudge.txt
echo "" | tee -a domjudge.txt
echo "configuration for spotboard" | tee -a domjudge.txt
echo "check /var/www/html/spotboard/src/config.js" | tee -a domjudge.txt
echo "" | tee -a domjudge.txt
